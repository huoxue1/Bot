...
func (con *Connect) SelctSign(event go_mybots.Event) int {
	con.Exist(event)
	var sign Sign
...
func (con *Connect) Exist(event go_mybots.Event) {
	var sign Sign
	err := con.Db.Get(&sign, "select * from sign where userId=?", strconv.Itoa(event.UserId))
	if err != nil {
...
/*
	return:
		...
		未签到： false
*/
func (con *Connect) IsSign(event go_mybots.Event) bool {
	con.Exist(event)
	var sign Sign
...
func (con *Connect) Update(n int, event go_mybots.Event) {
	con.Exist(event)
	if n > 0 {
